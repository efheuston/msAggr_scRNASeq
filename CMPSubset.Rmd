...
VizDimLoadings(seurat.object, dims = 1:10, nfeatures = 30, reduction = "pca", ncol = 2)
```

```{r}
pca.df <- seurat.object[["pca"]]
pca.df <- as.data.frame(as.matrix(slot(object = pca.df, name = "feature.loadings")))
print(seurat.object[["pca"]], dims = 2, nfeatures = 5)
...
FeaturePlot(seurat.object, features = surface.markers, pt.size = 1, split.by = "clust.ID", ncol = 1)
```
Save as png
```{r}
