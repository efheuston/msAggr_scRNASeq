...
for (meta.col in colnames(seurat.object@meta.data)){
	if(grepl(pattern = "RNA_snn_res", x = meta.col)==TRUE | grepl(pattern = "orig.ident", x = meta.col)==TRUE){
		myplot <- DimPlot(seurat.object, 
			...
			ggtitle(paste0(projectName, " dim", ndims, "res.", gsub("RNA_snn_res.", "", meta.col) ))
		plot(myplot)
		png(filename = paste0(projectName, " dim", ndims, "res.", gsub("RNA_snn_res.", "", meta.col), "-umap.png"), height = 800, width = 800, bg = "transparent")
		plot(myplot)
		dev.off()
		myplot <- DimPlot(seurat.object, 
											group.by = meta.col,
		...
		png(filename = paste0(projectName, "dim", ndims, "res.", gsub("RNA_snn_res.", "", meta.col), "-umap_FacetRes.png"), height = 800, width = 800, bg = "transparent")
		plot(myplot)
		dev.off()
...
```
Plot all black cells
```{r}
png(filename = paste0(projectName, " dim", ndims, "-umap.png"), height = 800, width = 800, bg = "transparent")
DimPlot(seurat.object, cols = c("black", "black", "black", "black"), group.by = "orig.ident") + NoLegend()
dev.off()
```

Plot gene expression
```{r}
feature.list <- c("Meis1", "Gata2", "Gata1", "Irf8", "Elane", "Fli1", "Zfpm1")

for(gene in feature.list){
	png(filename = paste0(projectName, "dim", ndims, "-FeaturePlot-", gene, ".png"), height = 400, width = 400)
	plot(FeaturePlot(seurat.object, features = gene))
	dev.off()
}
...
ident.list <- c("RNA_snn_res.1")
# ident.list <- c("RNA_snn_res.0.5")
cut_off <- 1.2
# ident.list <- c("RNA_snn_res.0.5", "RNA_snn_res.1")

load(file = "/Users/heustonef/Desktop/10XGenomicsData/msAggr_scRNASeq/RFunctions/mouseIDTable.rda")

# for(tested.ident in ident.list){
# 	ident.res <- gsub("RNA_snn_res", "", tested.ident)
# 	seurat.object <- SetIdent(seurat.object, value = tested.ident)
# 	all.markers <- FindAllMarkers(seurat.object)
all.markers <- data.frame(all.markers %>% group_by(cluster))
top100 <- data.frame(all.markers %>% group_by(cluster) %>% top_n(n = 100, wt = absValue))
	xlsx::write.xlsx(x = all.markers[,c("avg_log2FC", "p_val_adj", "cluster", "gene")],
									 file = paste0(projectName, "_FindALLMarkers_dim",ndims, ".xlsx"),
									 sheetName = paste0("res", ident.res),
									 col.names = TRUE,
									 row.names = FALSE,
									 append = TRUE)
		xlsx::write.xlsx(x = data.frame(top100[,c("avg_log2FC", "p_val_adj", "cluster", "gene")]),
									 file = paste0(projectName, "_FindALLMarkers_dim",ndims, "-top100.xlsx"),
									 sheetName = 'top100',
									 col.names = TRUE,
									 row.names = FALSE,
									 append = FALSE)
	...
	HGNC <- with(mouseIDTable, mouseIDTable$HGNC[match(all.markers$gene, mouseIDTable$MGI)])
	all.markers$HGNC <- HGNC
	sig.res <- all.markers[all.markers$p_val_adj <= 0.05, ]
	...
	for(cluster in unique(sig.res$cluster)){
		...
		try(dir.create(paste0("RankList_res", ident.res, "_findAll_hgnc/"), showWarnings = FALSE))
		write.table(new.table, file = paste0("RankList_res", ident.res, "_findAll_hgnc/res", ident.res, "_cluster", cluster, ".rnk"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t", )
		# write.table(new.table[new.table$avg_log2FC >= cut_off,], file = paste0("RankList_res", ident.res, "_findAll_hgnc/res", ident.res, "_cluster", cluster, "-co", cut_off, "up.rnk"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t", )
		# write.table(new.table[new.table$avg_log2FC >= (cut_off)*-1,], file = paste0("RankList_res", ident.res, "_findAll_hgnc/res", ident.res, "_cluster", cluster, "-co", cut_off, "dn.rnk"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t", )
		}

		#top 100		
	HGNC <- with(mouseIDTable, mouseIDTable$HGNC[match(top100$gene, mouseIDTable$MGI)])
	top100$HGNC <- HGNC
	sig.res <- top100[top100$p_val_adj <= 0.05, ]
	sig.res <- sig.res[c("avg_log2FC", "HGNC", "cluster")]
	sig.res <- sig.res[!(sig.res$HGNC == "" | is.na(sig.res$HGNC)),] # GSEA will fail if there are any blanks or NAs in the table
	sig.res <- sig.res[]
	...
	for(cluster in unique(sig.res$cluster)){
		...
		new.table <- sig.res[sig.res$cluster == cluster, c("HGNC", "avg_log2FC")]
		new.table <- new.table[order(-new.table$avg_log2FC), ]
		try(dir.create(paste0("RankList_res", ident.res, "_findAll_hgnc_top100/"), showWarnings = FALSE))
		write.table(new.table, file = paste0("RankList_res", ident.res, "_findAll_hgnc_top100/res", ident.res, "_cluster", cluster, "-top100.rnk"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t", )
		# write.table(new.table[new.table$avg_log2FC >= cut_off,], file = paste0("RankList_res", ident.res, "_findAll_hgnc_top100/res", ident.res, "_cluster", cluster, "-co", cut_off, "top100up.rnk"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t", )
		# write.table(new.table[new.table$avg_log2FC >= (cut_off)*-1,], file = paste0("RankList_res", ident.res, "_findAll_hgnc_top100/res", ident.res, "_cluster", cluster, "-co", cut_off, "top100dn.rnk"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t", )
}
```

## topGO with FindAllMarkers
```{r}
source("/Users/heustonef/Desktop/10XGenomicsData/msAggr_scRNASeq/RFunctions/GoEnrichmentAnalysis.R")
# GO_enrichment_function(my_folder = "RankList_res.0.5_findAll_hgnc/", file.pattern = ".rnk", id.type = "symbol")
GO_enrichment_function(my_folder = "RankList_res.1_findAll_hgnc/", file.pattern = ".rnk", id.type = "symbol")
GO_enrichment_function(my_folder = "backups/RankList_res.1_findAll_hgnc/", file.pattern = "up.rnk", id.type = "symbol")
```

write table of cluster IDs
```{r}
write.table(seurat.object@meta.data, file = "msAggr_SeuratMetaData.txt", row.names = row.names(seurat.object@meta.data), sep = "\t", col.names = TRUE, quote = FALSE)
```
...
seurat.object <- FindNeighbors(seurat.object, dims = 1:ndims)
# seurat.object <- FindClusters(seurat.object, resolution = 0.5)
seurat.object <- RunUMAP(seurat.object, dims = 1: ndims, n.components = 3)
```
...
```{r}
my_object <- seurat.object@reductions$umap@cell.embeddings
...
plot3d(x=my_object[,1], y=my_object[,2], z=my_object[,3], 
			 col = color.palette[seurat.object@meta.data$orig.ident], 
...
legend3d("topright", c(levels(seurat.object@meta.data$orig.ident)), col = color.palette, pch = 19, cex = 1)
# biomark.embeddings <- subset(tech.integrated, subset = orig.ident == "biomark")
...
movie3d(spin3d(axis = c(1, 1, 1), rpm = 6), duration = 10, dir = getwd(), clean = TRUE, webshot = FALSE, movie = paste0(projectName, "_dim", ndims, "umap_movie-orig.ident"))
```
...
bg3d("white", alpha = 0)
plot3d(x=my_object[,1], y=my_object[,2], z=my_object[,3], 
			 col = color.palette[seurat.object@meta.data$RNA_snn_res.0.5], 
			 box=FALSE, axes=FALSE, xlab = "", ylab = "", zlab = "")
rgl.lines(c(-10, 10), c(0, 0), c(0, 0), color = "black")
rgl.lines(c(0, 0), c(-10, 10), c(0, 0), color = "red")
rgl.lines(c(0, 0), c(0, 0), c(-10, 10), color = "green")
legend3d("topright", c(levels(seurat.object@meta.data$RNA_snn_res.0.5)), col = color.palette, pch = 19, cex = 1)
rgl.viewpoint(zoom = .5)
movie3d(spin3d(axis = c(1, 1, 1), rpm = 6), duration = 10, dir = getwd(), clean = TRUE, webshot = FALSE, movie = paste0(projectName, "_dim", ndims, "res.0.5-umap_movie"))
