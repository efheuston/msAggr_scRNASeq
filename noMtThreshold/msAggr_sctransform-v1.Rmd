...
########IS SCTRANSFORM MORE ROBUST FOR DIFFERENT SEQUENCING DEPTHS??#########
...
## Save base object
```{r}
saveRDS(seurat.object, file = "msAggr_rawSeurat.rds")
```
...
```{r}
DimPlot(seurat.object, group.by = "orig.ident", reduction = "umap")
```
...
```{r}
fav.genes <- c("Gata1", "Gata2", "Pf4", "Dntt", "Mpo", "Meis1", "Irf8", "Elane", "Fli1", "Zfpm1", "proc")
```

```{r}
FeaturePlot(seurat.object, features = fav.genes, ncol = 3)
```
...
One thing I've probably done in the past is regress out cell cycle factors. Did not do that here. Repeat analysis but include cell cycle regression.  
Cell cycle scoring comes after `RunPCA()`

Step 1: load in raw object
```{r}
seurat.object <- readRDS("msAggr_rawSeurat.rds")
```

Since `SCTransform()` is best for different correcting different sequencing depths, going to stick with that

The [Cell Cycle Regression Vignette](https://satijalab.org/seurat/articles/cell_cycle_vignette.html) says to do `NormalizeData()`, `FindVariableFeatures()`, `ScaleData()`, `RunPCA()`, `CellCycleScoring()`, then `RunPCA()` with s.genes and g2m.genes. It seems to be more complicated with `SCTransform()` because you don't want to normalize normalized data, but you need normalized data to properly run `CellCycleScoring()`. Found discussion board and [this](https://github.com/satijalab/seurat/issues/1679#issuecomment-557781838) is the answer that makes the most sense to me
...
```{r}
seurat.object <- SCTransform(seurat.object, assay = 'RNA', new.assay.name = "SCT", method = "glmGamPoi", vars.to.regress = c("nFeature_RNA", "nCount_RNA"))
```

Cell cycle scoring
```{r}
seurat.object <- CellCycleScoring(seurat.object, s.features = cc.genes$s.genes, g2m.features = cc.genes$g2m.genes, assay = 'SCT', set.ident = FALSE)
```
