...
```{r}
ident.list <- c(paste0("integrated_snn_res", object.res))
...
for(cluster in unique(sig.res$cluster)){
	...
	try(write.table(new.table, file = paste0("RankList_findAll_hgnc/", projectName, "pctVar", pct.var, "res", object.res, "cluster", cluster, ".rnk"), quote = FALSE, row.names = FALSE, col.names = TRUE, sep = "\t", ))
...
```
```{r}
png(filename = paste0(projectName, " pctVar", fav.var, "res.", object.res, "-fluor.png"), height = 2400, width = 800)
RidgePlot(biomark.integrated, features = fluor.cols, cols = color.palette, ncol = 1)
dev.off()
```
