...
```{r}
ident.list <- c(paste0("integrated_snn_res", integrated.res))
...
```
```{r}
png(filename = paste0(projectName, " pctVar", integrated.var, "res.", integrated.res, "-fluor.png"), height = 2400, width = 800)
RidgePlot(biomark.integrated, features = fluor.cols, cols = color.palette, ncol = 1)
dev.off()
```
...
```{r}
seurat.object <- AddMetaData(seurat.object, metadata = tech.integrated@meta.data[, ident.list])
```
			...
			if(grepl(pattern = target.col.pattern, x = target.col) == TRUE){
				...
				for(col.id in colnames(enrich.df)){
					...
					for(row.id in rownames(enrich.df)){
						...
						enrich.df[row.id, col.id] <- num.x
					}
				}
				if(!sum(rowSums(enrich.df)) == nrow(seurat.object@meta.data)){print("error! Not all cells have been assigned to a category!")}
				colnames(enrich.df) <- sapply(colnames(enrich.df), function(x) paste0(test.col.pattern, "_", x))
				rownames(enrich.df) <- sapply(rownames(enrich.df), function(x) paste0(target.col.pattern, "_", x))
				print(enrich.df)
			}
		}
	}
...
```{r}
for(test.col in colnames(seurat.object@meta.data)){
	if(grepl(pattern = test.col.pattern, x = test.col)){
		test.col.id <- sort(as.numeric(levels(seurat.object@meta.data[[test.col]])))
		for (target.col in colnames(seurat.object@meta.data)){
			if(grepl(pattern = target.col.pattern, x = target.col)){
				match.col.id <- sort(as.numeric(levels(seurat.object@meta.data[[target.col]])))}
				enrich.df <- data.frame(matrix(ncol = length(test.col.id), nrow = length(match.col.id)))
				colnames(enrich.df) <- test.col.id
				rownames(enrich.df) <- match.col.id
				meta.df <- seurat.object@meta.data
				for(row.id in rownames(enrich.df)){
					tot.clus <- nrow(meta.df[meta.df[[target.col]] == row.id,])
					for(col.id in colnames(enrich.df)){
						num.x <- nrow(meta.df[(meta.df[[target.col]] == row.id) & (meta.df$clust.ID == col.id),])
				...
				if(!is.null(xlsx_filename)){
					xlsx::write.xlsx(enrich.df, file = xlsx_filename, append = TRUE)}
