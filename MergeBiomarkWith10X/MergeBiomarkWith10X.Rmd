...
tech.integrated <- FindNeighbors(tech.integrated, dims = 1:ndims)
tech.integrated <- FindClusters(tech.integrated, resolution = 0.5)
tech.integrated <- RunUMAP(tech.integrated, dims = 1: ndims, n.components = 3)
```
Plot 3D UMAP

```{r}
my_object <- tech.integrated@reductions$umap@cell.embeddings
...
plot3d(x=my_object[,1], y=my_object[,2], z=my_object[,3], 
			 col = color.palette[tech.integrated@active.ident], 
...
movie3d(spin3d(axis = c(1, 1, 1), rpm = 6), duration = 10, dir = getwd(), clean = TRUE, webshot = TRUE)
```
