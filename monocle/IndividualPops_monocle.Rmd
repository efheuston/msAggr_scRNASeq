...
```{r}
seurat.rds <- paste0(cell.pop, "Subpop_pctVar", seurat.var.pct, ".RDS")
projectName <- paste0(cell.pop, "Subpop_monocle" )
set.seed(seed.value)
```
...
for(colID in colnames(colData(cds))){
	if(grepl("umap_res", colID)){
		...
		# save plot
		png(filename = paste0(projectName,  "-", colID, ".png"), height= 800, width = 800)
		plot(cluster_plot)
		dev.off()
	}
}
...
```{r}
marker_test_res <- top_markers(cds, group_cells_by = "umap_res_0.001", reference_cells = 1000, cores = 8)
```
```{r}
marker_test_res
```
...
## save result
```{r}
saveRDS(cds, file = paste0(projectName, "-umap_res0.001_psdtime.RDS"))
```
...
my_plot <- plot_cells(cds, reduction_method = "UMAP", 
					...
					scale_color_manual(values = color.palette) +
					ggtitle(paste0(projectName, "-umap_res0.001_psdtime"))
plot(my_plot)

png(filename = paste0(projectName, "-umap_res0.001_psdtime.png"), height = 800, width = 800)
plot(my_plot)
dev.off()
```
...
```{r}
my_plot <- plot_cells(cds, reduction_method = "UMAP", 
            genes = c("Elane", "Fli1", "Gata1", "Gata2", "Klf1", "Meis1", "Zfpm1", "Dntt", "Cd150"), trajectory_graph_color = "red",
            label_groups_by_cluster = FALSE, label_leaves = FALSE, label_branch_points = TRUE,  
            cell_size = 1, group_label_size = 8, graph_label_size = 5, trajectory_graph_segment_size = 1)
plot(my_plot)

png(filename = paste0(projectName, "-umap_YFG_psdtime.png"), height = 1600, width = 2000, bg = "transparent")
plot(my_plot)
dev.off()
```
